...
var jsbox = function (_options) {
    ...
    var zh = document.documentElement.clientHeight || document.body.clientHeight;
    //
    var jsboxId = new Date().getTime();
    var options = ".jsbox" + jsboxId;
    var context = '#' + options_.onlyid;
    ...
    this.show = function () {
        ...
        //
        var wc = (options_.FixedLeft != false) ? options_.FixedLeft : (zw - options_.conw) / 2;
        //(options_.FixedLeft != false) ? wc = options_.FixedLeft : wc = zw / 2 - options_.conw / 2;
        var hc = (options_.FixedTop != false) ? options_.FixedTop : zh / 2 - 160;
        //(options_.FixedTop != false) ? hc = options_.FixedTop : hc = zh / 2 - 150;
        options_.buttonCon = (options_.buttonCon != false) ? options_.buttonCon : "确定";
        ...
        if (options_.Close == true) {
            var Close = '<a href="javascript:void(0)" title="关闭" class="jsbox-close">';
        } else {
            var Close = '<a href="javascript:void(0)" title="关闭" class="jsbox-close" style="display:none;">';
        }
        ...
        //包装层
        var con =
            "<div id='" + options_.onlyid + "' class='jsbox jsbox" + jsboxId + "'>" +
                "<div class='jsboxContent' style='overflow-x:hidden;width:" + options_.conw + "px;'></div>" +
            "</div>";
        //
        ...
        var loaddiv = $('<div class="loaddiv" style="display:block;"></div>');
        ...
        var content = $('<div class="loaddiv" style="display:block; height:auto;">' +
                            '<div class="tccCon">' +
                                '<div class="tccCon_t">' + options_.content + '</div>' +
                                '<div class="tccCon_a">' +
                                    '<div class="WJButton wj_blue tcQz">确定' + '</div>' +
        ...
        if (options_.url != false && options_.iframe != false) {
            ...
            if (options_.footer != false) {
                //$(".jsboxFooter", options).before(options_.content);
                $(".jsboxFooter", options).before(content);
            } else {
                //$('.jsboxContent', options).append(options_.content);
        ...
        $(context).off('click', '.jsboxAn_Cancel').on('click', '.jsboxAn_Cancel', function () {
            $(this).parents(options).remove();
            $('.' + options_.onlyid + '_lightBox').remove();
            $('body').css('overflow', 'auto');
            //需要再次绑定body click事件
            //if (options_.textEdit) {
            //    $('body').one('click', options_.textEdit, options_.textEdit.bodyClick);
            //}
        });

        $(context).off('click', '.jsbox-close').on('click', '.jsbox-close', function () {
            $(this).parents(options).remove();
            $('.' + options_.onlyid + '_lightBox').remove();
            $('body').css('overflow', 'auto');
            //需要再次绑定body click事件
            if (options_.textEdit) {
                $('body').one('click', options_.textEdit, options_.textEdit.bodyClick);
                //important!!!防止bodyClick立即被触发
                return false;
            }
        });

        $(context).off('click', '#save').on('click', '#save', function () {
            //关闭对话框，移除元素
            $(this).parents(options).remove();
            $('.' + options_.onlyid + '_lightBox').remove();
            //需要再次绑定body click事件
        ...
        //弹出框滚动时禁止背景滚动
        $('.jsboxContent').live('mousewheel', function (e) {
            designScroll();
        });
